...
module Text.Reprinter.Examples.WeakSourceCoherence where
...
data Op = Add Bool Span deriving (Data, Eq, Show, Typeable)
...
refactorLoop src2src ast = if new == ast then ast else refactorLoop src2src new
  where
    new = src2src ast
