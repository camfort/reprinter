...
module ReprinterWeakSpec where
...
spec :: Spec
spec = do
  describe "refactor multiplication by 4 to bit-shift by 2" $ do
    it "test" $ refactor "y=x*4\n" `shouldBe` "y=x<<2\n"
    it "test" $ refactor "y=x*x*4\n" `shouldBe` "y=x*x<<2\n"
    it "test" $ refactor "y=4*x\n" `shouldBe` "y=x<<2\n"

    it "2" $ do
      refactor "someVar  = 1    *        0     *  x * y\n"
      `shouldBe`
      "someVar  = 1    `add`        0     `add`  x `add` y\n"
    -- it "3" $ do
...
instance Refactorable Op where
  isRefactored (Mult True _) = Just Replace
  isRefactored (ShiftL True _) = Just Replace
  isRefactored _ = Nothing

  getSpan (Mult _ s) = s
  getSpan (ShiftL _ s) = s
...
refactorZeroOnce = map (\(Decl s n e) -> (Decl s n (go e)))
  where
    go (App _ s (Mult _ _) e (Const _ _ 0)) = markRefactored (go e) s
    go (App _ s (Mult _ _) (Const _ _ 0) e) = markRefactored (go e) s
...
-- Apply the refactoring in a loop until two consecutive refactorings produce an identical output.
-- Ensures "deep" refactoring, e.g. in conjuction with refactorZero
refactorLoop :: (AST -> AST) -> AST -> AST
refactorLoop refactorFun ast = if new == ast then ast else refactorLoop refactorFun new
  where new = refactorFun ast

-- Mark nodes as refactored, for testing
refactorMult :: AST -> AST
refactorMult = map (\(Decl s n e) -> (Decl s n (go e)))
  where
    -- go (App _ s (Mult _ s') (Const _ s'' 4) e2) = App True s (ShiftL True s') e2 (Const _ s'' 2)
    go (App _ s (Mult _ s') e1 (Const _ s'' 4)) = App True s (ShiftL True s') e1 (Const True s'' 2)
    go a = a
  ...
  where
    continueExpr expr = do
        ...
        if isPlus then do
          ...
          return $ App False (lb,ub) (Mult False (p1,p2)) expr expr'
        else return expr
